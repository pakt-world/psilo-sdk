...
export class AuthService implements AuthModuleType {
  ...
  public async paktWeb3Login(privateKey: string): Promise<string> {
    ...
    const { message, tempToken: challengeToken } =
      await this.web3AuthRequest(address);

    const signedMessage = await wallet.signMessage(message);
    const validateResult = await this.web3AuthValidate(signedMessage, challengeToken);
    ...
    await this.web3AuthOnboard(
      validateResult.tempToken,
      address,
      address,
      `${address.toLowerCase()}@pakt.internal`,
    );
    ...
    const finalResult = await this.web3AuthValidate(signedMessage2, challengeToken2);
  ...
  public async web3AuthRequest(account: string): Promise<PaktWeb3RequestResponse> {
  ...
  public async web3AuthValidate(
    signedMessage: string,
    tempToken: string,
    tokenId?: string,
  ): Promise<PaktWeb3ValidateResponse> {
    const raw = await this.connector.post<any>("/v1/auth/web3/validate", { signedMessage, tempToken, ...(tokenId && { tokenId }) });
  ...
  public async web3AuthOnboard(
    ...
    return this.connector.post<PaktWeb3OnboardResponse>(
      ...
      { tempToken, firstName, lastName, email }
    );
  }
}
